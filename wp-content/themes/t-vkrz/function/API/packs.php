...
function init_new_classement ($data) {

  $id_top  = $data['id_top'];
  $id_user = $data['id_user'];
  $classement_array = array();
...
function get_ranking_json ($data) {
...
function get_allflammes () {

  $allflammesarray = array();

  $flammes = new WP_Query(array(
    'ignore_sticky_posts'     => true,
    'update_post_meta_cache'  => false,
    'no_found_rows'           => true,
    'post_type'               => 'categorie',
  ...
  if ($flammes->have_posts()) {

    while ($flammes->have_posts()) : $flammes->the_post();

      $id_flamme = get_the_ID();
      $list_contenders_ids = array();
      $list_contenders     = array();
      $list_contenders_ids = get_field('liste_des_contenders_flamme', $id_flamme);

      $i = 0;
      foreach ($list_contenders_ids as $contender) {
        array_push($list_contenders, array(
          "id"                => $i,
          "id_wp"             => $contender,
          "cover"             => get_the_post_thumbnail_url($contender, 'medium'),
          "name"              => get_the_title($contender),
          "complement"        => get_field('complement_contender', $contender),
          "more_to"           => array(),
          "less_to"           => array(),
          "place"             => 0,
          "ratio"             => 0,
      ...
      array_push($allflammesarray, array(
        'id_flamme'   => $id_flamme,
        'url_flamme'  => get_the_permalink($id_flamme),
        'name_flamme' => get_the_title($id_flamme),
        'contenders'  => $list_contenders
      ));
