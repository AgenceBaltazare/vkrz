  ...
  .on("shown.bs.modal", function () {
    ...
    const cropImage = function(cropper, canvas, imageName, type) {
      ...
      if(type === "contender" && contenders.some(contender => contender.contenderName === imageName)) {
        alert("Tu as déjà envoyé ce Contender ;)");
      ...
      // const storageRef = ref(storage, `${topTitle.value}-${uniqueFolderId}/${imageName}-${generateUniqueId()}.png`);
      const storageRef = ref(storage, `${imageName}-${generateUniqueId()}.png`);
    ...
    cropAndSendBtn.addEventListener("click", function () {
      ...
      if(isTopToSend) {
        ...
        if(topTitle.value) {
          ...
          alert("Please fill in all the fields from before!")
        }
...
soumettreContenders.addEventListener('click', async (e) => {
  ...
  if(!contenderImgInput.value) {
    ...
    alertMsg.innerHTML = `
      <i class="fa fa-exclamation-triangle" aria-hidden="true"></i> Try to put more than 2 contenders! 🤭
    `;
    alertMsg.classList.remove('d-none')
  } else {
    ...
    contenders.forEach(contender => {
      $.ajax({
        url: "http://localhost:8888/vkrz/wp-json/vkrz/v1/addcontenderfromcreatetop",
        method: "POST",
        data: {
    ...
    // await fetch('https://toplistbattle.com:3002/discord', {
    //   method: 'POST',
    //   headers: {
