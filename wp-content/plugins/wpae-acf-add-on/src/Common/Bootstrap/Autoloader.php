...
namespace Pmae\Common\Bootstrap;
...
class Autoloader
{
    ...
    {
        // register helpers
        if (is_dir($this->rootDir . '/helpers')) foreach (\PMAE_Helper::safe_glob($this->rootDir . '/helpers/*.php', \PMAE_Helper::GLOB_RECURSE | \PMAE_Helper::GLOB_PATH) as $filePath) {
    ...
    private function registerActions()
    {
        ...
        if (is_dir($this->rootDir . '/actions')) if (is_dir($this->rootDir . '/actions')) foreach (\PMAE_Helper::safe_glob($this->rootDir . '/actions/*.php', \PMAE_Helper::GLOB_RECURSE | \PMAE_Helper::GLOB_PATH) as $filePath) {
            require_once $filePath;
    ...
    {
        if (is_dir($this->rootDir . '/filters')) foreach (\PMAE_Helper::safe_glob($this->rootDir . '/filters/*.php', \PMAE_Helper::GLOB_RECURSE | \PMAE_Helper::GLOB_PATH) as $filePath) {
            require_once $filePath;
            $function = $actionName = basename($filePath, '.php');
            if (preg_match('%^(.+?)[_-](\d+)$%', $actionName, $m)) {
                $actionName = $m[1];
    ...
    private function registerShortcodes()
    {
        ...
        if (is_dir($this->rootDir . '/shortcodes')) foreach (\PMAE_Helper::safe_glob($this->rootDir . '/shortcodes/*.php', \PMAE_Helper::GLOB_RECURSE | \PMAE_Helper::GLOB_PATH) as $filePath) {
