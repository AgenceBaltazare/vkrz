...
class PMAE_Model_Record extends PMXE_Model {
	...
	public function getRelated($model, $keyAssoc = NULL) {
		...
		if ( ! empty($this->id)) {
			if (is_null($keyAssoc)) {
				$defaultPrefix = strtolower(preg_replace('%^' . strtoupper(PMAE_Plugin::PREFIX) . '|_Record$%', '', get_class($this)));
				$keyAssoc = array();
		...
		return $related instanceof PMAE_Model_List ? $related->convertRecords() : $related;
	}
