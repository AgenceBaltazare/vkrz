 ...
 */
abstract class PMAE_Controller {
	/**
	...
	protected function render($viewPath = null) {
		...
		$filePath = PMAE_Plugin::ROOT_DIR . '/views/' . $viewPath;
		if (is_file($filePath)) {
			extract($this->data);
			include $filePath;
		} else {
	...
	protected function error($msgs = NULL) {
		...
		$viewPathRel = str_replace('_', '/', preg_replace('%^' . preg_quote(PMAE_Plugin::PREFIX, '%') . '%', '', strtolower(get_class($this)))) . '/error.php';
		if (is_file(PMAE_Plugin::ROOT_DIR . '/views/' . $viewPathRel)) { // if calling controller class has specific error view
