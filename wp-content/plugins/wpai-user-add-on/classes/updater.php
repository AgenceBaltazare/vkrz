...
if( ! class_exists('PMUI_Updater') ) {
    ...
    class PMUI_Updater {
        ...
        public function plugin_row_meta( $links, $file ) {
            if ( $file == $this->name ) {
                $row_meta = array(
                    'changelog'    => '<a href="' . admin_url( 'plugin-install.php?tab=plugin-information&plugin=wpai-user-add-on&section=changelog&TB_iframe=true&width=772&height=477' ) . '" class="thickbox open-plugin-details-modal" title="' . esc_attr( __( 'View WP All Import - User Add-On Changelog', 'wp_all_import_user_add_on' ) ) . '">' . __( 'Changelog', 'wp_all_import_user_add_on' ) . '</a>',
                );
