<?php
/**
 * Customize API: UR_Customize_Select2_Control class
 *
 ...
 */
class UR_Customize_Select2_Control extends WP_Customize_Control {
	...
	public function content_template() {
		...
		<select class="ur-select2" {{{data.link}}} {{{data.inputAttrs}}} >
	...
	public function get_google_fonts() {
		$google_fonts = get_transient( 'ur_google_fonts' );
		...
		if ( false === $google_fonts ) {
			...
			if ( ! is_wp_error( $raw_google_fonts ) ) {
				...
				if ( isset( $google_fonts->items ) ) {
					...
					set_transient( 'ur_google_fonts', $google_fonts, WEEK_IN_SECONDS );
				}
			}
		}

		return apply_filters( 'user_registration_extensions_sections', $google_fonts );
	}
}
