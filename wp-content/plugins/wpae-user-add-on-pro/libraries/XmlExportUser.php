...
if (!class_exists('XmlExportUser')) {
    ...
    final class XmlExportUser
    {
        ...
        public static function prepare_import_template($exportOptions, &$templateOptions, $element_name, $ID, &$cf_list = '') {
            ...
            if (XmlExportUser::$is_export_shop_customer) {
                $templateOptions['custom_type'] = 'shop_customer';
                $template_key = 'pmsci_customer';
                $templateOptions['pmui']['import_users'] = 0;
                $templateOptions[$template_key]['import_customers'] = 1;
                $templateOptions[$template_key]['shipping_source'] = 'import';
                $billing_fields = ['billing_first_name', 'billing_last_name', 'billing_company', 'billing_address_1', 'billing_address_2', 'billing_address_1', 'billing_city', 'billing_postcode', 'billing_country',
                    'billing_state', 'billing_email', 'billing_phone'];
                $shipping_fields = ['shipping_first_name', 'shipping_last_name', 'shipping_company', 'shipping_address_1', 'shipping_address_2', 'shipping_city', 'shipping_postcode',
                    'shipping_country', 'shipping_state'];
                $customer_fields = array_merge($billing_fields, $shipping_fields);
                $billing_fields_list = empty($templateOptions['pmsci_billing_fields_list']) ? [] : $templateOptions['pmsci_billing_fields_list'];
                $shipping_fields_list = empty($templateOptions['pmsci_shipping_fields_list']) ? [] : $templateOptions['pmsci_shipping_fields_list'];

                if ($element_type == 'cf') {
                    // Map Customer fields & remove them from custom fields list.
                    foreach ($customer_fields as $customer_field) {
                        if (str_replace('_', '', $customer_field) == $element_name) {
                            $templateOptions[$template_key][$customer_field] = '{' . $element_name . '[1]}';
                            foreach ($templateOptions['custom_name'] as $key => $meta_key) {
                                if ($meta_key == $customer_field) {
                                    unset($templateOptions['custom_name'][$key]);
                                    unset($templateOptions['custom_value'][$key]);
                                    unset($templateOptions['custom_format'][$key]);
                                    break;
                                }
                            }
                            // Remove billing & shipping fields from custom fields list.
                            if (in_array($customer_field, $cf_list)) {
                                foreach ($cf_list as $key => $cf) {
                                    if ($cf == $customer_field) {
                                        unset($cf_list[$key]);
                                        if (in_array($customer_field, $billing_fields) && !in_array($customer_field, $billing_fields_list)) {
                                            $billing_fields_list[] = $customer_field;
                                        }
                                        if (in_array($customer_field, $shipping_fields) && !in_array($customer_field, $shipping_fields_list)) {
                                            $shipping_fields_list[] = $customer_field;
                                        }
                                        break;
                                    }
                                }
                ...
                if ( ! empty($billing_fields_list) ) {
                    $templateOptions['pmsci_is_update_billing_fields'] = 1;
                    $templateOptions['pmsci_update_billing_fields_logic'] = 'only';
                    $templateOptions['pmsci_billing_fields_list'] = $billing_fields_list;
                    $templateOptions['pmsci_billing_fields_only_list'] = implode(',', $billing_fields_list);
                }
                else {
                    $templateOptions['pmsci_is_update_billing_fields'] = 0;
                }
                if ( ! empty($shipping_fields_list) ) {
                    $templateOptions['pmsci_is_update_shipping_fields'] = 1;
                    $templateOptions['pmsci_update_shipping_fields_logic'] = 'only';
                    $templateOptions['pmsci_shipping_fields_list'] = $shipping_fields_list;
                    $templateOptions['pmsci_shipping_fields_only_list'] = implode(',', $shipping_fields_list);
                }
                else {
                    $templateOptions['pmsci_is_update_shipping_fields'] = 0;
                }
            }
            ...
            switch ($element_type) {
                // Export Users
                case 'user_login':
                    $templateOptions[$template_key]['login'] = '{' . $element_name . '[1]}';
                    $templateOptions['is_update_login'] = 1;
                    break;
                case 'user_pass':
                    $templateOptions[$template_key]['pass'] = '{' . $element_name . '[1]}';
                    $templateOptions['is_update_password'] = 1;
					$templateOptions['is_hashed_wordpress_password'] = 1;
                    break;
                case 'user_nicename':
                    $templateOptions[$template_key]['nicename'] = '{' . $element_name . '[1]}';
                    $templateOptions['is_update_nicename'] = 1;
                    break;
                case 'user_email':
                    $templateOptions[$template_key]['email'] = '{' . $element_name . '[1]}';
                    $templateOptions['is_update_email'] = 1;
                    break;
                case 'user_registered':
                    $templateOptions[$template_key]['registered'] = '{' . $element_name . '[1]}';
                    $templateOptions['is_update_registered'] = 1;
                    break;
                case 'display_name':
                    $templateOptions[$template_key]['display_name'] = '{' . $element_name . '[1]}';
                    $templateOptions['is_update_display_name'] = 1;
                    break;
                case 'user_url':
                    $templateOptions[$template_key]['url'] = '{' . $element_name . '[1]}';
                    $templateOptions['is_update_url'] = 1;
                    break;
                case 'first_name':
                    $templateOptions[$template_key]['first_name'] = '{' . $element_name . '[1]}';
                    $templateOptions['is_update_first_name'] = 1;
                    break;
                case 'last_name':
                    $templateOptions[$template_key]['last_name'] = '{' . $element_name . '[1]}';
                    $templateOptions['is_update_last_name'] = 1;
                    break;
                case 'wp_capabilities':
                    $templateOptions['is_update_role'] = 1;
                    $templateOptions[$template_key]['role'] = '{' . $element_name . '[1]}';
                    break;
                case 'nickname':
                    $templateOptions[$template_key]['nickname'] = '{' . $element_name . '[1]}';
                    $templateOptions['is_update_nickname'] = 1;
                    break;
                case 'description':
                    $templateOptions[$template_key]['description'] = '{' . $element_name . '[1]}';
